<?php
	...
	class Usuario
	{
		...
		public function atualizarUsuario($nome,$senha,$imagem){
			$sql = MySql::concectar()->prepare("UPDATE `tb_admin.usuarios` SET nome = ?,password = ?,img= ? WHERE user = ?");
		...
		public static function userExist($user){
			$sql = MySql::concectar()->prepare("SELECT `id` FROM `tb_admin.usuarios` WHERE user=?");
		...
		public static function cadastrarUsuario($user,$senha,$imagem,$nome,$cargo){
			$sql = MySql::concectar()->prepare("INSERT INTO `tb_admin.usuarios` VALUES (null,?,?,?,?,?)");
